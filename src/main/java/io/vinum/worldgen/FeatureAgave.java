...
public class FeatureAgave extends Feature<NoFeatureConfig> {
    ...
    public boolean check(IWorld world, BlockPos pos){
        ...
        if(bsPosDown.getBlock().isIn(BlockTags.SAND) && bsPos.getMaterial().isReplaceable() && bsPos != Blocks.WATER.getDefaultState() && bsPos != Blocks.LAVA.getDefaultState()){
            return true;
        }
        else{
            return false;
    ...
    public void generateAgaveFeature(IWorld world, BlockPos pos, Random random, int type){
        world.setBlockState(pos, ModBlocks.STEEL_BRAZIER.get().getDefaultState(), 2);
    }
